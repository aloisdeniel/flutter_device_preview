  ...
  void open() {
    if (!_isOpen) {
      ...
      final startPosition = Offset(
        _key.absolutePosition!.left - widget.parentBounds.left,
        _key.absolutePosition!.top - widget.parentBounds.top,
      );
...
class __PopOverContainerState extends State<_PopOverContainer>
  ...
  Widget build(BuildContext context) {
    ...
    final parentBounds = widget.parentBounds;
    final isHorizontal =
    ...
    final baseTranslate = (_translate ?? Offset.zero);
    return Positioned(
      left: bounds.left + baseTranslate.dx,
      top: bounds.top - media.viewInsets.bottom + baseTranslate.dy,
      width: bounds.width,
      height: bounds.height,
      child: AnimatedOpacity(
        duration: duration,
        opacity: _isStarted ?? false ? 1.0 : 0.0,
